...
static SemaphoreHandle_t s_ble_hidh_sec_semaphore = NULL;
static void (*s_hidh_passkey_cb)(uint32_t passkey) = NULL;
...
esp_hidh_gattc_event_handler(struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_CONNECT:
        ...
        if (event->connect.status == 0) {
            ...
            if (!dev) {
                /* Try identity address (bonded reconnect uses identity addr) */
                dev = esp_hidh_dev_get_by_bda(desc.peer_id_addr.val);
            }
            if (!dev) {
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        if (!dev->connected) {
            ...
            dev->status = event->disconnect.reason;
            dev->ble.conn_id = -1;
            // Keep the device structure for reconnect -- don't free it.
            // Post CLOSE_EVENT so the app knows, but the device stays in
            // the list with in_use=true so reconnect can reuse it.
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
        if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
            ...
            ESP_LOGI(TAG, "Passkey display: %06lu", (unsigned long)pkey.passkey);
            if (s_hidh_passkey_cb) {
                s_hidh_passkey_cb(pkey.passkey);
            }
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
        ...
        if (s_ble_hidh_sec_semaphore) {
            xSemaphoreGive(s_ble_hidh_sec_semaphore);
        }
        return 0;
...
esp_err_t esp_ble_hidh_init(const esp_hidh_config_t *config)
{
    ...
    s_ble_hidh_sec_semaphore = xSemaphoreCreateBinary();
...
esp_hidh_dev_t *esp_ble_hidh_dev_open(uint8_t *bda, uint8_t address_type)
{
    ...
    // Drain any stale semaphore signals from a previous disconnect.
    // The old connection's GATT cleanup calls SEND_CB() for each pending
    // operation, which can leave multiple signals in the semaphore.
    while (xSemaphoreTake(s_ble_hidh_cb_semaphore, 0) == pdTRUE)
        ;
    if (s_ble_hidh_sec_semaphore)
        while (xSemaphoreTake(s_ble_hidh_sec_semaphore, 0) == pdTRUE)
            ;

    // Use explicit connection params with 5s supervision timeout
    // so we detect keyboard switching away quickly
    ...
    /* Initiate security/pairing before GATT discovery -- many HID devices
     * require encryption before allowing access to report characteristics. */
    ESP_LOGI(TAG, "Initiating BLE security on conn_handle=%d", dev->ble.conn_id);
    ret = ble_gap_security_initiate(dev->ble.conn_id);
    if (ret == 0) {
        /* Wait for encryption change (pairing + bonding to complete) */
        xSemaphoreTake(s_ble_hidh_sec_semaphore, pdMS_TO_TICKS(30000));
        ESP_LOGI(TAG, "Security complete, proceeding with GATT discovery");
    } else {
        ESP_LOGW(TAG, "Security initiation failed: %d (proceeding anyway)", ret);
    }
...
// Lightweight reconnect: reuse existing device, skip GATT discovery.
// Only re-establishes BLE link, re-initiates security, re-subscribes.
esp_hidh_dev_t *esp_ble_hidh_dev_reconnect(esp_hidh_dev_t *dev)
{
    ...
    if (!dev || !dev->in_use) {
        ESP_LOGE(TAG, "reconnect: invalid device");
        return NULL;
    }

    // Drain stale semaphore signals from the old connection
    while (xSemaphoreTake(s_ble_hidh_cb_semaphore, 0) == pdTRUE)
        ;
    if (s_ble_hidh_sec_semaphore)
        while (xSemaphoreTake(s_ble_hidh_sec_semaphore, 0) == pdTRUE)
    ...
    WAIT_CB();
    if (dev->ble.conn_id < 0) {
        ESP_LOGE(TAG, "reconnect: connection failed, status=0x%x", dev->status);
        return NULL;
    }

    ESP_LOGI(TAG, "Reconnected on conn_handle=%d, re-subscribing", dev->ble.conn_id);

    // Re-initiate security (encryption with bonded keys)
    ret = ble_gap_security_initiate(dev->ble.conn_id);
    if (ret == 0) {
        xSemaphoreTake(s_ble_hidh_sec_semaphore, pdMS_TO_TICKS(30000));
        ESP_LOGI(TAG, "Reconnect security complete");
    } else {
        ESP_LOGW(TAG, "Reconnect security initiation failed: %d", ret);
    }

    dev->connected = true;

    // Re-subscribe to report notifications (handles are still valid)
    attach_report_listeners(dev);
    ...
    if (event_loop_handle) {
        ...
        p.open.status = ESP_OK;
        p.open.dev = dev;
        esp_event_post_to(event_loop_handle, ESP_HIDH_EVENTS, ESP_HIDH_OPEN_EVENT,
                          &p, sizeof(esp_hidh_event_data_t), portMAX_DELAY);
    }
...
void esp_ble_hidh_set_passkey_cb(void (*cb)(uint32_t passkey))
{
    s_hidh_passkey_cb = cb;
}
